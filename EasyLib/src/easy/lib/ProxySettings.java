...
public class ProxySettings 
{
    ...
    private static boolean isSystemProxyValid(Context ctx)
    {
		...
		if (_proxyString != null && _proxyString != "" && _proxyString.contains(":")) return true;
		else return false;
    }
	...
	private static boolean testSystemProxy(Context ctx)
	{
		...
		{
			_proxyStatus = false;
			message = "invalid proxy settings";
		}
		else if (!isSystemProxyReachable(ctx))
		{
			_proxyStatus = false;
			message = "proxy server not reachable";
		}
		else if (!isInternetReachable(ctx))
		{
			_proxyStatus = false;
			message = "proxy net not reachable";
		}
		else
		{
			_proxyStatus = true;
	...
	{
		Toast.makeText(ctx, "enable proxy", Toast.LENGTH_SHORT).show();
		...
		if (_proxyStatus)
		{
			...
			Toast.makeText(ctx, "proxy enabled", Toast.LENGTH_LONG).show();
		}
		else
