...
public class AboutBrowser extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		btnAdvance.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				LayoutParams lp = (LayoutParams) advanceSettings.getLayoutParams();
				if (lp.height == 0) {// open the advance settings
	...
	protected void onPause() {
		if (resetDefault) {
			...
			editor.putInt("share_mode", shareMode.indexOfChild(findViewById(shareMode.getCheckedRadioButtonId())));
			...
			if (advancedOpened) {// only save advanced settings if it opened
				advancedOpened = false;
